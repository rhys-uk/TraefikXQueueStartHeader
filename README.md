...
This plugin injects a header into each incoming request with the current Unix time in milliseconds. It uses `strconv.FormatInt` under the hood to ensure minimal overhead and maximum performance on every request.
